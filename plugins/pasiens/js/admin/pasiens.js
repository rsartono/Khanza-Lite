...
$(document).ready(function(){
    $.ajax({
      type: 'GET',
      url: '{?=url()?}/admin/pasiens/ajax?show=propinsi&t={?=$_SESSION[token]?}',
...
$(document).on('click', '.pilihpropinsi', function (e) {
  ...
  $.ajax({
    type: 'GET',
    url: '{?=url()?}/admin/pasiens/ajax?show=kabupaten&kd_prop='+kd_prop+'&t={?=$_SESSION[token]?}',
    success: function(response) {
...
$(document).on('click', '.pilihkabupaten', function (e) {
  ...
  $.ajax({
    type: 'GET',
    url: '{?=url()?}/admin/pasiens/ajax?show=kecamatan&kd_kab='+kd_kab+'&t={?=$_SESSION[token]?}',
    success: function(response) {
...
$(document).on('click', '.pilihkecamatan', function (e) {
  ...
  $.ajax({
    type: 'GET',
    url: '{?=url()?}/admin/pasiens/ajax?show=kelurahan&kd_kec='+kd_kec+'&t={?=$_SESSION[token]?}',
    success: function(response) {
