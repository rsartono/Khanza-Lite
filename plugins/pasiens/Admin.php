...
class Admin extends AdminModule
{
    ...
    */
    public function getManage($page = 1)
    {
        ...
        $pagination = new \Systems\Lib\Pagination($page, count($totalRecords), 10, url([ADMIN, 'pasiens', 'manage', '%d']));
        ...
        if (count($rows)) {
            foreach ($rows as $row) {
                ...
                $row['editURL'] = url([ADMIN, 'pasiens', 'edit', $row['no_rkm_medis']]);
                $row['delURL']  = url([ADMIN, 'pasiens', 'delete', $row['no_rkm_medis']]);
                $row['viewURL'] = url([ADMIN, 'pasiens', 'view', $row['no_rkm_medis']]);
                $this->assign['list'][] = $row;
            }
        }

        return $this->draw('manage.html', ['pasiens' => $this->assign]);
    }
    ...
    */
    public function getAdd()
    {
        ...
        $this->assign['manageURL'] = url([ADMIN, 'pasiens', 'manage']);

        return $this->draw('form.html', ['pasiens' => $this->assign]);
    }
    ...
    */
    public function getEdit($no_rkm_medis)
    {
        ...
        if (!empty($pasien)) {
            ...
            redirect(url([ADMIN, 'pasiens', 'manage']));
        }
    }
    ...
    */
    ...
    {
        $this->_addHeaderFiles();
        $this->assign['print_rm'] = url([ADMIN, 'pasiens', 'print_rm', $no_rkm_medis]);
        $this->assign['print_kartu'] = url([ADMIN, 'pasiens', 'print_kartu', $no_rkm_medis]);
        $pasien = $this->db('pasien')->where('no_rkm_medis', $no_rkm_medis)->oneArray();

        if (!empty($pasien)) {
            $this->assign['view'] = $pasien;
            $this->assign['fotoURL'] = url('/plugins/pasiens/img/'.$pasien['jk'].'.png');

            $this->assign['manageURL'] = url([ADMIN, 'pasiens', 'manage']);

            return $this->draw('view.html', ['pasiens' => $this->assign]);
        } else {
            redirect(url([ADMIN, 'pasiens', 'manage']));
        }
    }
    ...
    */
    public function postSave($id = null)
    {
        ...
        if (!$no_rkm_medis) {
            $location = url([ADMIN, 'pasiens', 'add']);
        } else {
            $location = url([ADMIN, 'pasiens', 'edit', $no_rkm_medis]);
        }
    ...
    */
    ...
    public function getAjax()
    {
        ...
        switch($show){
          ...
          $kabupaten = $this->db('kabupaten')->where('kd_kab', $kode)->toArray();
          ...
          $kecamatan = $this->db('kecamatan')->where('kd_kec', $kode)->toArray();
          ...
          $kelurahan = $this->db('kelurahan')->where('kd_kel', $kode)->toArray();
    ...
    */
    public function getJavascript()
    {
        ...
        echo $this->draw(MODULES.'/pasiens/js/admin/pasiens.js');
        exit();
    }
    ...
    */
    public function getCss()
    {
        ...
        echo $this->draw(MODULES.'/pasiens/css/admin/pasiens.css');
        exit();
    }
    ...
    private function _addHeaderFiles()
    {
        ...
        // MODULE SCRIPTS
        $this->core->addCSS(url([ADMIN, 'pasiens', 'css']));
        $this->core->addJS(url([ADMIN, 'pasiens', 'javascript']), 'footer');
    }
