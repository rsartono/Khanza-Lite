...
class Admin extends AdminModule
{
    ...
    public function getPoliklinik($page = 1)
    {
        ...
        // pagination
        $totalRecords = $this->core->db('poliklinik')->like('kd_poli', '%'.$phrase.'%')->orLike('nm_poli', '%'.$phrase.'%')->orLike('status', 1)->toArray();
        ...
        // list
        $offset = $pagination->offset();
        $query = $this->db()->pdo()->prepare("SELECT * FROM poliklinik WHERE (kd_poli LIKE ? OR nm_poli LIKE ? OR status LIKE ?) LIMIT $perpage OFFSET $offset");
        $query->execute(['%'.$phrase.'%', '%'.$phrase.'%', 1]);
        $rows = $query->fetchAll();
